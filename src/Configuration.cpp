...
bool Configuration::isValidServerBlockPlacement(
    ...
    // Helper function to check if a Block has a "listen" directive
bool blockHasListenDirective(const Block& block) {
	return block.directives.find("listen") != block.directives.end();
}
...
bool Configuration::isServerHavePort() const {
    for (std::vector<Block>::const_iterator block_it = blocks.begin();
		...
		if (block_it->name == "http") {
			for (std::vector<Block>::const_iterator sub_block_it = block_it->sub_blocks.begin();
			 sub_block_it != block_it->sub_blocks.end(); ++sub_block_it) {
				if (sub_block_it->name == "server") {
					if (sub_block_it->directives.find("listen") == sub_block_it->directives.end()) {
						return false;
					}
				}
			}
...
// -------------------------- 정보 가져오는 함수 -------------------------------
Block Configuration::getServerBlockWithPortAndName(const std::string &port_number, const std::string &server_name) const {
	// port 번호와 server_name 일치하는 server 찾기
    for (std::vector<Block>::const_iterator block_it = blocks.begin(); block_it != blocks.end(); ++block_it) {
		if (block_it->name == "http") {
			for (std::vector<Block>::const_iterator sub_it = block_it->sub_blocks.begin(); sub_it != block_it->sub_blocks.end(); ++sub_it) {
				if (sub_it->name == "server" &&
					sub_it->directives.find("listen") != sub_it->directives.end() && sub_it->directives.at("listen") == port_number &&
					sub_it->directives.find("server_name") != sub_it->directives.end() && sub_it->directives.at("server_name") == server_name) {
					return *sub_it;
				}
			}
		}
...
Block Configuration::getLocationBlock(const std::string& path) const {
...
std::vector<std::string> Configuration::getCgiExtensions(const std::string& path) const {
	std::string server_name;
	std::string port_number;
	std::string location;
	...
	for (std::vector<Block>::const_iterator block_it = server.sub_blocks.begin(); block_it != server.sub_blocks.end(); ++block_it) {
		if (block_it->name.find("location ") != std::string::npos) {
			std::string location_name = block_it->name.substr(9);
			if (location_name[0] == '~') {
...
bool Configuration::isMethodAllowedFor(const std::string& path, const std::string& method) const {
	Block location_block = getLocationBlock(path);

	if (location_block.directives.find("limit_except") != location_block.directives.end()) {
		return location_block.directives["limit_except"].find(method) != std::string::npos;
	}
	return true;
}

std::string Configuration::getAllowedMethods(const std::string &path) const {
	Block location_block = getLocationBlock(path);

	if (location_block.directives.find("limit_except") != location_block.directives.end()) {
		return location_block.directives.at("limit_except");
	}
	return "";
}

bool Configuration::isDirectoryListingEnabled(const std::string& path) const {
	Block location_block = getLocationBlock(path);
	...
	if (location_block.directives.find("autoindex") != location_block.directives.end()) {
...
std::string Configuration::getErrorPageFromServer(const std::string& path) const {
	std::string server_name;
	std::string port_number;
	std::string location;
